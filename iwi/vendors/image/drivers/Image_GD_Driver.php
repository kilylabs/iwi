...
class Image_GD_Driver extends Image_Driver {
    ...
    public function resize($properties)
    {
        ...
        if ($properties['width'] > $width / 2 AND $properties['height'] > $height / 2)
        {
            ...
            if ($status = imagecopyresampled($img, $this->tmp_image, 0, 0, 0, 0, $pre_width, $pre_height, $width, $height))
            {
                // Swap the new image for the old one
                imagedestroy($this->tmp_image);
                $this->tmp_image = $img;
    ...
    protected function imagecreatetransparent($width, $height)
    {
        ...
        $ext = pathinfo($this->image_info['file'],PATHINFO_EXTENSION);
        switch($ext) {
        case 'png':
        case 'gif':
            imagecolortransparent($img, imagecolorallocatealpha($new, 0, 0, 0, 127));
            imagealphablending($img, false);
            imagesavealpha($img, true);
            //imagecopyresampled($img, $this->tmp_image, 0, 0, 0, 0, $width, $height, $width, $height);
        }
        return $img;
         */
        ...
        imagecopyresampled($img, self::$blank_png, 0, 0, 0, 0, $width, $height, self::$blank_png_width, self::$blank_png_height);
